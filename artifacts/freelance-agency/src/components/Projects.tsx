      ...
      <div className="w-full h-full flex">
        <div className="w-12 h-full bg-slate-800 flex flex-col items-center py-3 gap-2.5">
          ...
          {[...Array(5)].map((_, i) => (
            <div
              key={i}
              className={`w-5 h-5 rounded-md ${i === 1 ? "bg-white/20" : "bg-white/8"}`}
            />
          ))}
        </div>
            ...
            <div className="flex-1 flex items-end gap-0.5 pt-1">
              {[35, 60, 45, 80, 55, 90, 70, 85, 60, 95].map((h, i) => (
                <div
                  ...
                  style={{
                    height: `${h}%`,
                    background: i === 9 ? "#3b82f6" : "rgba(255,255,255,0.12)",
                  }}
                />
              ))}
            </div>
        ...
        <div className="flex-1 bg-white p-3 flex flex-col gap-2">
          ...
          <div className="flex gap-2 mt-1">
            {[
              ...
              <div
                key={i}
                className={`flex-1 ${c} border rounded-lg p-1.5 flex flex-col gap-1`}
              >
    ...
    mockup: (
      <div className="w-full h-full flex flex-col">
        ...
        <div className="flex-1 bg-white p-2 grid grid-cols-2 gap-2">
          {["bg-rose-50", "bg-pink-50", "bg-fuchsia-50", "bg-purple-50"].map(
            (c, i) => (
              <div
                key={i}
                className={`${c} rounded-lg overflow-hidden flex flex-col`}
              >
      ...
      <div className="w-full h-full flex flex-col bg-neutral-950">
        ...
        <div className="flex-1 px-2 pb-2 grid grid-cols-3 gap-1.5">
          {[
            ...
            <div
              key={i}
              className={`rounded-lg bg-white/5 border ${s.border} p-1.5 flex flex-col gap-1 shadow-lg ${s.glow}`}
            >
    ...
    <section id="projects" className="py-24 bg-white">
      <div className="container mx-auto px-6 md:px-12">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-14 gap-6">
          ...
          >
            Premium websites built for ambitious brands, clinics, restaurants,
            gamers, and creators.
          </motion.p>
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map((project, index) => (
            ...
            >
              <div
                className={`relative h-52 w-full bg-gradient-to-br ${project.color} overflow-hidden`}
              >
              ...
              <div className="p-5 flex-1 flex flex-col">
                <div className="flex items-start justify-between gap-2 mb-2">
                  ...
                  <div
                    className={`w-2.5 h-2.5 rounded-full shrink-0 mt-1 ${project.accent}`}
                  />
                </div>
                ...
                <div className="flex flex-wrap gap-1.5 mt-auto">
                  {project.tags.map((tag) => (
                    <span
                      key={tag}
                      className="px-2.5 py-1 bg-neutral-50 text-neutral-500 border border-neutral-100 rounded-full text-xs font-medium"
                    >
                      {tag}
                    </span>
                  ))}
